...
class userModel {
  ...
  static addUser = async (userData, next) => {
    ...
    try {
      const result = await pool.query(findQuery, [id, email, gender, password, role])
  ...
  static updateUser = async (id, userData, next) => {
    ...
    const findQuery = 'UPDATE users SET email = $1, gender = $2, password = $3, role = $4 WHERE id = $5'

    try {
      const result = await pool.query(findQuery, [email, gender, password, role, id])
