...
class movieModel {
  ...
  static addMovie = async (userData, next) => {
    ...
    try {
      const result = await pool.query(findQuery, [id, title, genres, year, photo])
  ...
  static updateMovie = async (id, userData, next) => {
    ...
    const findQuery = 'UPDATE movies SET title = $1, genres = $2, year = $3, photo = $4 WHERE id = $5'

    try {
      const result = await pool.query(findQuery, [title, genres, year, photo, id])
